...
Run with pandas installed:
python airbnb_deliverable_4.py [input.csv] --output-dir cleaned
The original CSV is never overwritten. IDs are read as text to avoid rounding.
"""
import argparse
...
- Input SHA-256: `{hashlib.sha256(source.read_bytes()).hexdigest()}`.
...
if __name__ == "__main__":
    ...
    parser.add_argument("input", nargs="?", default="christchurch_listings_2025-10_to_2026-06.csv")
    parser.add_argument("--output-dir", default="cleaned")
    args = parser.parse_args()
    clean(args.input, args.output_dir)
